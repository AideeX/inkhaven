'use client';

import { useRouter } from 'next/navigation';
import { useUsers } from '@/app/lib/hooks/fetchusers';
import Image from 'next/image';

const CommunityMembers: React.FC = () => {
    const { users, loading, error } = useUsers();
    const router = useRouter();

    if (loading) return <p>Loading community members...</p>;
    if (error) return <p>Error loading community members: {error.message}</p>;

    const displayedUsers = users.slice(0, 6);

    const handleNavigation = (url: string) => {
        router.push(url);
    };

    return (
        <div className="bg-light-secondaryBg dark:bg-dark-secondaryBg p-4 rounded-lg shadow-md mt-8">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-light-heading dark:text-dark-heading">Community Members</h2>
                <span
                    className="text-light-accentMedium dark:text-dark-accentMedium hover:underline text-sm cursor-pointer"
                    onClick={() => handleNavigation('/members')}
                >
                    View All
                </span>
            </div>
            <div className="grid grid-cols-2 gap-4">
                {displayedUsers.map(user => (
                    <div
                        key={user.id}
                        className="flex items-center space-x-4 hover:bg-light-accentLight dark:hover:bg-dark-accentLight p-2 rounded cursor-pointer"
                        onClick={() => handleNavigation(`/home/${user.id}/user/profile`)}
                    >
                        <Image
                            src={user.photoURL || '/default-profile.png'}
                            width={40}
                            height={40}
                            alt={user.displayName}
                            className="w-12 h-12 object-cover rounded-full"
                        />
                        <span className="text-light-text dark:text-dark-text font-medium">{user.displayName}</span>
                    </div>
                ))}
            </div>
        </div>
    );
};

export default CommunityMembers;